...
rem WORK_HOME="Script running : SecureImageGen Directory(.\scripts\secure_img_gen_02.bat"
rem ROOT KEY TYPE = 0x0(ROM fixed), 0x1(HashedPublickey), 0x2(HMACKEY)

set ROOT_KEY_TYPE=0x00000002
set ROLLBACK_COUNTER=0x00000001
set ROOT_ECDSAKEY=../../KeyGen/keys/ecdsa_privatekey.pem
set ROOT_AESKEY=0
set ROOT_HMACKEY=../../KeyGen/keys/hmac_rootkey.bin
set ROOT_HASHGEN=0
...
echo %SIGN_TOOL% %DST_BIN% %SRC_BIN% %ROOT_ECDSAKEY% %ROOT_HMACKEY% %ROOT_KEY_TYPE% %ROLLBACK_COUNTER% %ROOT_HASHGEN% %ROOT_HASHOFPUBLICKEY%

%SIGN_TOOL% %DST_BIN% %SRC_BIN% %ROOT_ECDSAKEY% %ROOT_HMACKEY% %ROOT_KEY_TYPE% %ROLLBACK_COUNTER% %ROOT_HASHGEN% %ROOT_HASHOFPUBLICKEY%
